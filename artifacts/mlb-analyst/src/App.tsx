      ...
      {query.isLoading ? <LoadingPanel rows={6} /> : query.isError ? <QueryMessage kind="error" onRetry={() => query.refetch()} /> : !data ? <QueryMessage kind="empty" /> : (
        <>
          ...
          <div className="health-summary">
            ...
            <Metric label="Issues requiring review" value={(data.issues?.length ?? 0) >= 50 ? '50+' : data.issues?.length ?? 0} note={(data.issues?.length ?? 0) >= 50 ? `${criticalCount ? `${criticalCount} critical in list · ` : ''}List capped at 50; totals may be higher` : criticalCount ? `${criticalCount} critical` : 'No critical issues in list'} tone={criticalCount ? 'bad' : data.issues?.length ? 'warn' : 'good'} />
          </div>
