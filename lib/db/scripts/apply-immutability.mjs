 ...
 *   4. Partial unique index enforcing exactly one ACTIVE model per market.
 *   5. Append-only guard on game_weather_observations.
 *
...
const statements = [
  ...
  `
  CREATE OR REPLACE FUNCTION prevent_historical_source_coverage_mutation()
  RETURNS TRIGGER LANGUAGE plpgsql AS $$
  BEGIN
    RAISE EXCEPTION
      'historical_source_coverage is append-only. '
      'A retry or correction must write a new range receipt.';
  END;
  $$
  `,
  ...
  `
  CREATE OR REPLACE FUNCTION prevent_batter_pitcher_snapshot_mutation()
  RETURNS TRIGGER LANGUAGE plpgsql AS $$
  BEGIN
    IF current_setting('app.bypass_immutability', true) = 'true' THEN
      RETURN OLD;
    END IF;
    RAISE EXCEPTION
      'Batter-versus-pitcher research history is append-only. Corrections must create a new snapshot or source event row.';
  END;
  $$
  `,
  ...
  `DROP TRIGGER IF EXISTS batter_pitcher_events_immutable ON batter_pitcher_events;
   CREATE TRIGGER batter_pitcher_events_immutable
   BEFORE UPDATE OR DELETE ON batter_pitcher_events
   FOR EACH ROW EXECUTE FUNCTION prevent_batter_pitcher_snapshot_mutation();`,

  `DROP INDEX IF EXISTS batter_pitcher_source_event_idx;
   CREATE UNIQUE INDEX IF NOT EXISTS batter_pitcher_source_event_revision_idx
   ON batter_pitcher_events (source_id, source_event_key, content_checksum);`,

  `DROP TRIGGER IF EXISTS batter_pitcher_snapshots_immutable ON batter_pitcher_snapshots;
   CREATE TRIGGER batter_pitcher_snapshots_immutable
   BEFORE UPDATE OR DELETE ON batter_pitcher_snapshots
   FOR EACH ROW EXECUTE FUNCTION prevent_batter_pitcher_snapshot_mutation();`,

  `DROP TRIGGER IF EXISTS batter_pitch_type_snapshots_immutable ON batter_pitch_type_snapshots;
   CREATE TRIGGER batter_pitch_type_snapshots_immutable
   BEFORE UPDATE OR DELETE ON batter_pitch_type_snapshots
   FOR EACH ROW EXECUTE FUNCTION prevent_batter_pitcher_snapshot_mutation();`,

  `DROP TRIGGER IF EXISTS batter_pitch_type_features_immutable ON batter_pitch_type_features;
   CREATE TRIGGER batter_pitch_type_features_immutable
   BEFORE UPDATE OR DELETE ON batter_pitch_type_features
   FOR EACH ROW EXECUTE FUNCTION prevent_batter_pitcher_snapshot_mutation();`,

  // Historical intelligence retains raw observations and shared game context
  // before any profile derivation. Replays and official corrections append a
  // revision; they may never rewrite what a prior pregame analysis could see.
  `
  CREATE OR REPLACE FUNCTION prevent_historical_intelligence_mutation()
  RETURNS TRIGGER LANGUAGE plpgsql AS $$
  BEGIN
    RAISE EXCEPTION
      'Historical intelligence source facts and derived profiles are append-only. Insert a revision instead of updating or deleting.';
  END;
  $$
  `,
  `DROP TRIGGER IF EXISTS historical_game_contexts_append_only ON historical_game_contexts;
   CREATE TRIGGER historical_game_contexts_append_only
   BEFORE UPDATE OR DELETE ON historical_game_contexts
   FOR EACH ROW EXECUTE FUNCTION prevent_historical_intelligence_mutation();`,
  `DROP TRIGGER IF EXISTS historical_source_coverage_append_only ON historical_source_coverage;
   CREATE TRIGGER historical_source_coverage_append_only
   BEFORE UPDATE OR DELETE ON historical_source_coverage
   FOR EACH ROW EXECUTE FUNCTION prevent_historical_source_coverage_mutation();`,
  `DROP TRIGGER IF EXISTS historical_player_observations_append_only ON historical_player_observations;
   CREATE TRIGGER historical_player_observations_append_only
   BEFORE UPDATE OR DELETE ON historical_player_observations
   FOR EACH ROW EXECUTE FUNCTION prevent_historical_intelligence_mutation();`,
  `DROP TRIGGER IF EXISTS player_intelligence_features_append_only ON player_intelligence_features;
   CREATE TRIGGER player_intelligence_features_append_only
   BEFORE UPDATE OR DELETE ON player_intelligence_features
   FOR EACH ROW EXECUTE FUNCTION prevent_historical_intelligence_mutation();`,
  ...
  `,
  // ── Remediation task 4.2: weather observations are append-only ───────────
  //
  // Weather is forecast data and it changes. A post-freeze weather change must
  // be visible AS a change, so an observation is never updated in place.
  `
  CREATE OR REPLACE FUNCTION prevent_game_weather_mutation()
  RETURNS TRIGGER LANGUAGE plpgsql AS $$
  BEGIN
    RAISE EXCEPTION
      'game_weather_observations is append-only. A changed forecast is a new observation.';
  END;
  $$
  `,
  `DROP TRIGGER IF EXISTS game_weather_append_only ON game_weather_observations`,
  `
  CREATE TRIGGER game_weather_append_only
  BEFORE UPDATE OR DELETE ON game_weather_observations
  FOR EACH ROW EXECUTE FUNCTION prevent_game_weather_mutation()
  `,
  `DROP TRIGGER IF EXISTS game_weather_truncate_guard ON game_weather_observations`,
  `
  CREATE TRIGGER game_weather_truncate_guard
  BEFORE TRUNCATE ON game_weather_observations
  FOR EACH STATEMENT EXECUTE FUNCTION prevent_game_weather_mutation()
  `,

  // ── Remediation task 1.5: exactly one ACTIVE model per market ────────────
  //
  // Enforced at the database level, not only in promoteModelVersion. Two live
  // models for one market would mean two different probabilities served for the
  // same row depending on which one loadActiveModels happened to pick.
  `
  CREATE UNIQUE INDEX IF NOT EXISTS model_versions_one_active_per_market_idx
  ON model_versions (market)
  WHERE status = 'ACTIVE'
  `,
