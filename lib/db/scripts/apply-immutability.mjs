...
const statements = [
  ...
  BEGIN
    ...
    IF NEW.status = 'ACTIVE' THEN
      /*
       * Phase 5B owns the controlled activation procedure. Do not rely on a
       * caller-controlled PostgreSQL setting here: any ordinary writer can set
       * one. Until that procedure and its validated walk-forward record exist,
       * activation is intentionally impossible.
       */
      RAISE EXCEPTION
        'Phase 5A candidates cannot be activated. Phase 5B must activate via its controlled walk-forward acceptance procedure.';
    END IF;
