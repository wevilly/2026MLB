    ...
    <div className="page-content rise-in">
      ...
      {query.isLoading ? <LoadingPanel rows={6} /> : query.isError ? <QueryMessage kind="error" onRetry={() => query.refetch()} /> : !data ? <QueryMessage kind="empty" /> : (
        <>
          ...
          <Panel>
            <SectionHeading eyebrow="Current player eligibility" title="Slate identity coverage" detail="Pregame coverage is measured from FantasyPros projected starters and lineups; MLB final-state records remain historical context only." />
            <div className="metric-grid">
              ...
              <Metric label="MLB final-state lineups" value={(coverage?.officialLineupPlayersTotal ?? 0) === 0 ? 'N/A' : `${coverage?.officialLineupPlayersMapped ?? 0}/${coverage?.officialLineupPlayersTotal ?? 0}`} note={(coverage?.officialLineupPlayersTotal ?? 0) === 0 ? 'NO POSTGAME MLB LINEUPS' : 'Historical settlement coverage'} tone={(coverage?.officialLineupPlayersTotal ?? 0) === 0 ? 'neutral' : (coverage?.officialLineupPlayersMapped ?? 0) === (coverage?.officialLineupPlayersTotal ?? 0) ? 'good' : 'bad'} />
