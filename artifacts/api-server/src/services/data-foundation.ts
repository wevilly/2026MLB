...
async function upsertStarter(person: JsonObject, teamId: number, gamePk: number, state: string, raw: JsonObject) {
  ...
  await pool.query(
    ...
    [gamePk, teamId, playerId, state, MLB_SOURCE, raw],
  );
  return true;
}
...
export async function ingestFantasyPros(requestedDate: string) {
  ...
  try {
    ...
    const getJson = async (path: string, params: Record<string, string>) => {
      ...
      const payload = await response.json() as JsonObject;
      if (!response.ok) throw new Error(`FantasyPros ${path} returned HTTP ${response.status}`);
      return { payload, status: response.status };
    };
    const [hitters, pitchers, lineups, news, playerDirectoryResponse] = await Promise.all([
    ...
    for (const item of [
      ...
      for (const row of rows) {
        ...
        if (!resolvedPlayerId) missingIdentity.add(sourcePlayerId);
        await pool.query(
          `INSERT INTO fantasypros_projection_rows (snapshot_id, source_player_id, canonical_player_id, team_abbreviation, position, projected_stats, raw_row, identity_confidence)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
          [
            snapshotId,
            sourcePlayerId,
    ...
    await recordIssue(
      FANTASY_PROS_SOURCE,
      ingestRunId,
      "LINEUP_NORMALIZATION_PENDING",
      "INFO",
      `${asArray(lineups.payload.games).length} FantasyPros lineup payloads stored as immutable raw records; canonical game mapping is pending.`,
    );
    await pool.query(
      `UPDATE ingest_issues SET resolved_at = now()
       WHERE source_id = $1 AND issue_type = 'SOURCE_FAILURE' AND resolved_at IS NULL`,
      [FANTASY_PROS_SOURCE],
    );
