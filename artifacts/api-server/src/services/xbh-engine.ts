...
function computeEvidenceScore(
  ...
  // Daily XBH/PA is the permitted API-backed market signal. Do not mix it
  // with unavailable contact-quality or historical pitcher metrics.
  if (xbhPerPa !== null) {
    ...
    return score + weatherAdjustment("XBH", weather).adjustment;
  }
...
async function writeCandidate(
  ...
  const result = await executor.query<{ candidate_id: string }>(
    ...
    [
      ...
      "RANK_DONT_GATE: independent ordinal rank from source-backed matchup research; FantasyPros values are not ranking inputs.",
      ingestRunId,
    ],
  );
